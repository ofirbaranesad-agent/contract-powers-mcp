/**
 * contract-powers — the tools, and nothing else.
 *
 * This file is the single source of truth for what the server can answer.
 * Two transports import it and add nothing to the answers:
 *   - src/index.js          stdio, for `npx -y github:ofirbaranesad-agent/contract-powers-mcp`
 *   - ~/site/app/mcp.mjs    streamable HTTP, for https://agent.zbang.net/api/mcp
 * The HTTP host does not hold a copy: it imports this exact file through a
 * symlink, so the two transports cannot answer differently.
 *
...
/**
 * A host that already holds the data locally can answer without a round trip.
 * The hosted HTTP transport runs inside the same container as the registry and
 * is blocked at the network layer from its own public domain, so for it this is
 * not an optimisation — it is the only way to read its own data.
 * Signature: (path) => parsed JSON, same shape getJson returns.
 */
let localSource = null;
export function useLocalSource(fn) { localSource = fn; }

/** Fetch JSON with a timeout, and never throw a bare network error at the model. */
async function getJson(path) {
  if (localSource) {
    try { return await localSource(path); }
    catch (err) { return { ok: false, error: `local read of ${path} failed: ${err.message}` }; }
  }
  const url = `${BASE}${path}`;
...
export const SERVER_INFO = { name: 'contract-powers', version: '1.1.0' };

export const INSTRUCTIONS =
  'Answers one question about an EVM contract on Base, Ethereum or Polygon: who can still change ' +
  'it, and what can they do to holders. list_watched_contracts and get_contract_powers are free and ' +
  'complete for contracts already in the registry; check_any_contract is a free preview for any other ' +
  'verified address, with the decision fields withheld and named. Every claim carries its evidence. ' +
  'None of it is an audit: a power listed here may be governed, timelocked or renounced in ways this ' +
  'does not model. Built and maintained by an autonomous AI agent operated by Ofir Baranes.';

export { TOOLS, handleTool, NOT_AN_AUDIT };
